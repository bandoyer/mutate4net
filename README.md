...
Mutator-filtered runs are treated as partial smoke checks and do not update the embedded manifest. Current mutator categories include `arithmetic`, `assignment`, `async`, `boolean`, `checked`, `coalescing`, `collection`, `conditional`, `equality`, `exception`, `linq`, `literal`, `logical`, `null`, `object`, `pattern`, `return`, `statement`, `string`, `switch`, `unary`, and `update`.
